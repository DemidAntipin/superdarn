...
point_input = layers.Input(shape=(1,4), name="point_input")
...
point_decoder = tf.keras.models.Model(inputs=point_input, outputs={"point_abs":point_abs, "point_phase": point_phase}, name="Point_decoder")
point_decoder.compile(optimizer=tf.keras.optimizers.Adam(learning_rate=1e-3), loss={"point_abs": abs_qty, "point_phase": phase_qty}, metrics={"point_abs":abs_qty, "point_phase": phase_qty})
...
try:
    ...
    for file in files:
        for chunk in pd.read_csv(file, compression='bz2', header=None, sep=r" ", chunksize=1000):
            ...
            data=create_tf_dataset(chunk, DatasetType.DECODER_POINT)
            train_data, val_data = tf.keras.utils.split_dataset(data, left_size=0.8)
            train_data = train_data.batch(32)
            val_data = val_data.batch(32)
            point_decoder.fit(train_data, validation_data=val_data, epochs=30, callbacks=[es])
except Exception as e:
    print(e)
point_decoder.save("Models/decoder_point.keras")
